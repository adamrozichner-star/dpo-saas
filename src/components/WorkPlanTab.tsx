...
import { CheckCircle2, Clock, AlertTriangle, ChevronDown, ChevronUp, Calendar, Filter } from 'lucide-react'
...
export default function WorkPlanTab() {
  ...
  const loadPlan = async () => {
    try {
      ...
      console.error('Failed to load work plan:', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-stone-800">תוכנית עבודה שנתית {year}</h2>
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ...
          { key: 'review', label: 'סקירות' },
          { key: 'audit', label: 'ביקורות' },
        ].map(f => (
          <button
            key={f.key}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-0.5">
                    ...
                    <span className="text-xs text-stone-400">{categoryLabels[task.category]}</span>
                  </div>
                </div>
